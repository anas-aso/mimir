...
func TestMultiPropagator_AddToCarrier_HappyPath(t *testing.T) {
	c := MapCarrier{}
	...
	err := p.AddToCarrier(ctx, c)
	require.NoError(t, err)
	require.Equal(t, "value-1", c.Get("key-1"))
	require.Equal(t, "value-2", c.Get("key-2"))
}

func TestMultiPropagator_AddToCarrier_Error(t *testing.T) {
	c := MapCarrier{}
	...
	err := p.AddToCarrier(context.Background(), c)
	require.EqualError(t, err, "something went wrong")
}
...
func (p *testPropagator) AddToCarrier(ctx context.Context, carrier Carrier) error {
	if p.err != nil {
		return p.err
	}

	carrier.Set(p.key, ctx.Value(p.key).(string))
	return nil
}
